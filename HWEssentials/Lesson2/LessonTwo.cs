...
public class LessonTwo : ILesson
{
    ...
    public void Run()
    {
        ...
        for (var i = 5; i < 14; i++) doubleList.AddNode(i);
        Console.WriteLine(doubleList.ToString());
        Console.WriteLine();

        Console.WriteLine("б) Выбираем ноды с индексами 0, 3, 8");
        Console.WriteLine(doubleList.GetNodeByIndex(0).Value + " " + doubleList.GetNodeByIndex(3).Value + " " +
                          doubleList.GetNodeByIndex(8).Value);
        Console.WriteLine();

        Console.WriteLine("в) вставляем ноды с полезно нагрузкой 333 после нодов с индексами 0 3 8");
        var node = doubleList.GetNodeByIndex(0);
        ...
        Console.WriteLine(
            "г) вставляем ноды с полезно нагрузкой 111 после нодов с индексами 0 4 10, но другим способом");
