...
public static class Lesson1Task3
{
    ...
    {
        if (member < 0) throw new ArgumentOutOfRangeException("Номер члена последовательнсти должен быть больше 0");
    ...
    public static void CalcFibonacciSquence()
    {
        ...
        for (var i = 0; i <= maxNumber; i++)
            Console.WriteLine(
                $"Номер {i} число с рекурсией {FibonacciByRecursion(i)}  без рекурсии {FibonacciByCicle(i)}");
    }
}